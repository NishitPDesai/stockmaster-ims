...
export function OperationDetails({ operation, onClose, onStatusChange }: OperationDetailsProps) {
  ...
  const getStatusActions = () => {
    const actions: Array<{ label: string; status: OperationStatus; variant: 'default' | 'destructive' }> = []
    
    if (operation.status === OperationStatus.DRAFT && canValidate) {
      actions.push({ label: 'Validate', status: OperationStatus.READY, variant: 'default' })
    }
    if (operation.status === OperationStatus.READY && canComplete) {
      actions.push({ label: 'Complete', status: OperationStatus.DONE, variant: 'default' })
    }
    // Only managers can cancel operations
    if (operation.status !== OperationStatus.DONE && operation.status !== OperationStatus.CANCELED && canCancel && user?.role === 'manager') {
      actions.push({ label: 'Cancel', status: OperationStatus.CANCELED, variant: 'destructive' })
    }
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle>Operation Details - {operation.documentNumber}</DialogTitle>
        ...
        <div className="space-y-6">
          ...
          {operation.documentType === 'TRANSFER' ? (
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm">{operation.sourceWarehouseName || '-'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-muted-foreground">Destination Warehouse</label>
                <p className="text-sm">{operation.destinationWarehouseName || '-'}</p>
              </div>
            </div>
          ) : (
            <div>
                ...
                <tbody>
                  {operation.lineItems.map((item) => (
                    <tr key={item.id} className="border-t">
                      ...
                      <td className="px-4 py-2 text-sm font-mono">{item.productSku}</td>
          ...
          {operation.validatedAt && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-muted-foreground">Validated At</label>
                <p className="text-sm">{formatDateTime(operation.validatedAt)}</p>
              </div>
              <div>
