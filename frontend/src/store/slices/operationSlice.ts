...
export const fetchOperations = createAsyncThunk(
  ...
  async (filters: OperationFilters | undefined, { rejectWithValue }) => {
    try {
      ...
      const response = await apiClient.get<Operation[]>('/operations', { params: filters })
...
export const fetchOperationById = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to fetch operation')
    }
  }
)
...
export const createOperation = createAsyncThunk(
  ...
  async (data: CreateOperationDto, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to create operation')
    }
  }
)
...
export const updateOperation = createAsyncThunk(
  'operations/update',
  async ({ id, data }: { id: string; data: UpdateOperationDto }, { rejectWithValue }) => {
    try {
      if (USE_MOCK) {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to update operation')
    }
  }
)
...
export const { setFilters, clearFilters, setSelectedOperation } = operationSlice.actions
